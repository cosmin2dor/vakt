...
export interface paths {
  ...
  '/parse': {
    ...
    put?: never
    /**
     * Parse a single line of vault text.
     * @description The sole parsing authority (CLAUDE.md): every semantic question about a line — is it valid, what does it mean, when does it next fire — is answered here, never client-side (SDD.md §2.1). Scoped to one line rather than a whole file: task identity in this codebase is already line-scoped (SDD.md G1-G4), and the Smart Editor's own consumers — live linting while typing, the cron popover's next-runs preview — both operate on the line the caret is in, not the whole file.
     */
    post: operations['parseLine']
    delete?: never
    options?: never
    head?: never
    patch?: never
...
export interface components {
  schemas: {
    ...
    /** @description One line of raw vault text, exactly as the editor's current line reads — checkbox, prose, and directives all included. */
    ParseRequest: {
      line: string
    }
    /** @description One @name(value) match, mirroring directive.Span plus the typed value directive.ParseSpan derives from it (SDD.md §2.5). Offsets are byte positions into the request's line, Go slice convention. value_type and the typed fields are null when name is not a registered directive. */
    ParsedSpan: {
      name: string
      start: number
      end: number
      ...
      value_start: number
      value_end: number
      /** @description The unparsed text between the parens. */
      raw_value: string
      /** @description string | enum | cron | datetime | integer. */
      value_type?: string | null
      /** @description Populated for value_type string or enum. */
      str?: string | null
      /**
       * Format: int64
       * @description Populated for value_type integer.
       */
      int?: number | null
      /**
       * Format: date-time
       * @description Populated for value_type datetime.
       */
      time?: string | null
      /** @description The five cron fields, populated for value_type cron. */
      cron?: string[] | null
    }
    /** @description A problem found in the parsed line. Deliberately not the Task- level Diagnostic schema: that one identifies a file and line number for a diagnostic raised against the whole vault, while this one identifies a character range within the single line the client just sent, which is what the editor needs to underline the right span while typing (SDD.md §2.1). */
    ParseDiagnostic: {
      /** @enum {string} */
      severity: 'warning' | 'error'
      message: string
      /** @description Machine-readable classification, e.g. unknown_directive. */
      code?: string | null
      /** @description Byte offset into the line where the problem starts. */
      start: number
      /** @description Byte offset into the line where the problem ends. */
      end: number
    }
    /** @description Everything the Smart Editor needs to render one line without evaluating any of it itself (CLAUDE.md: the backend is the only parser). */
    ParseResult: {
      spans: components['schemas']['ParsedSpan'][]
      diagnostics: components['schemas']['ParseDiagnostic'][]
      /** @description The next few times the line's @schedule or @once would fire, soonest first — an array rather than a single next_fire because the cron popover's preview (issues/milestone5.md, implement-contextual-helpers) shows several upcoming occurrences, not one. Null when the line has no valid @schedule/@once. */
      upcoming_fires?: string[] | null
    }
...
export interface operations {
  ...
  parseLine: {
    ...
    responses: {
      /** @description The line's typed spans, diagnostics, and schedule preview. */
      200: {
        headers: {
          [name: string]: unknown
        }
